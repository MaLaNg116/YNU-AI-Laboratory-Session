...
async function GetRandomForest(dim) {
  emit('loaded', false)
  await axios.get(`${URL}/GetRandomForest/${dim}`).then(res => {
    // 终止加载动画
    emit('loaded', true)
...
<template>
  <div>
    ...
    <div class="predict-area">
      <DataInputbox
          :URL="`${URL}/GetRandomForest/`"
          :pca_dim="pca_dim"
      ></DataInputbox>
    </div>
    <div class="text-area">
      <span class="content">&emsp;&emsp;对随机森林分类器的参数{{
          Object.keys(best_param).toString()
        }}进行了交叉验证，max_depth参数用于控制随机森林中每棵决策树的最大深度，较大的max_depth值允许模型学习更复杂的关系，但也可能导致过拟合。n_estimators参数指定了随机森林中决策树的数量，增加树的数量通常会提高模型的性能。验证范围为{{
          params
        }}。</span>
      <br/>
