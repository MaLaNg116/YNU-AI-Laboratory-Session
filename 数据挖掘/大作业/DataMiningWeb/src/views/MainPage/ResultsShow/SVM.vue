...
async function GetSVM(dim) {
  emit('loaded', false)
  await axios.get(`${URL}/GetSVM/${dim}`).then(res => {
    // 终止加载动画
    emit('loaded', true)
...
<template>
  <div>
    ...
    <div class="text-area">
      <span class="content">&emsp;&emsp;对SVM分类器的参数{{
          Object.keys(best_param).toString()
        }}进行了交叉验证，C用于控制误分类的惩罚程度，gamma用于控制数据映射到高维特征空间的复杂程度，kernel是用于将数据映射到高维特征空间的函数类型。验证范围为{{ params }}。</span>
      <br/>
