...
async function GetNaiveBayes(dim) {
  emit('loaded', false)
  await axios.get(`${URL}/GetNaiveBayes/${dim}`).then(res => {
    // 终止加载动画
    emit('loaded', true)
...
<template>
  <div>
    ...
    <div class="predict-area">
      <DataInputbox
          :URL="`${URL}/GetNaiveBayes/`"
          :pca_dim="pca_dim"
      ></DataInputbox>
    </div>
    <div class="text-area">
      <span class="content">&emsp;&emsp;对贝叶斯分类器的参数{{
          Object.keys(best_param).toString()
        }}进行了交叉验证，该参数用于平滑模型中的概率估计，它主要用于处理特征值在训练集中没有出现的情况，以避免在计算概率时遇到零除问题。验证范围为{{ params }}。</span>
      <br/>
